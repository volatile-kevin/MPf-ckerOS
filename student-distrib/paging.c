...
int32_t vid_map(uint8_t **screen_start) {
    ...
    page_directory[USER_MAP_LOC / FOUR_MB] =
            (unsigned int) user_video_page_table | 7; // User level permissions, 4kb page, r/w, present
    user_video_page_table[cur_terminal] =
            (unsigned int) VIDEO_MEM | 7; // User level permissions, 4kb page, r/w, present
...
void map_video_page(uint8_t terminal) {
    page_table[VIDEO_MEM_INDEX] =
            (VIDEO_MEM_INDEX + terminal) * FOUR_KB | 3; // Supervisor level permissions, 4kb page, r/w, present
    flush_tlb();
}
